<template>
  <ul v-if="projects.length" role="list" class="divide-y divide-gray-100">
    <li v-for="project in projects" :key="project.documentId"
      ...
      <div class="flex basis-1/2 flex-row items-center">
        <div class="pr-6">
          ...
          <ExclamationTriangleIcon v-else-if="project.condition === 'amber'" class="h-8 w-8 text-yellow-600"
            aria-hidden="true" />
        ...
        <div class="flex flex-col">
          <div class="flex items-start gap-x-3">
            ...
            <p v-if="project.stage === 'Awaiting Allocation'"
      ...
      <div class="flex flex-col basis-1/3 w-full sm:w-auto">
        <div v-if="isFinite(project.burndown)">
          ...
          <div class="my-2 relative w-full" aria-hidden="true">
            <div class="overflow-hidden rounded-full bg-gray-200">
              <div class="h-4 rounded-full" :class="[project.burndown > 105 ? 'bg-red-700' : 'bg-cyan-600']"
                :style="{ width: project.burndown + '%' }"></div>
              <div v-if="isFinite(project.anticipatedProgressBurndown)">
                <div class="group absolute -top-1 h-6 border-l-2 border-black"
                  :style="{ left: project.anticipatedProgressBurndown + '%' }">
                  <div
                    class="hidden group-hover:block absolute bottom-full mb-2 -translate-x-1/2 left-0 whitespace-nowrap">
        ...
        <div class="mt-2 grid grid-cols-1 gap-1 text-[10px] sm:text-xs leading-5 text-gray-600">
          <div class="flex items-center gap-x-1.5">
            ...
            <span>{{ project.daysBilled || 0 }} / {{ Math.round(Number(Duration.fromISO(project.estimate).toFormat('h'))/7.4) || 0 }} </span>
          </div>

          <div class="flex items-center gap-x-1.5">
            <span class="font-semibold text-gray-900">Budget consumed:</span>
            <span>£{{ project.moneyBilled?.toLocaleString() || 0 }} / £{{  project.amount?.toLocaleString() || 0 }}</span>
          </div>
        </div>
      </div>
      <div class="flex basis-1/6 justify-end">
        <div class="flex -space-x-0.5">
          ...
          <dd v-for="rse in project.rses" :key="rse.documentId">
            <div class="group flex relative">
              <img class="h-8 w-8 rounded-full shadow-md bg-gray-50 ring-2 ring-white" :src="getImageUrl(rse.photo)"
                :alt="`${rse.firstname} ${rse.lastname}`" />
              <span class="z-10 group-hover:opacity-100 transition-opacity bg-gray-800 p-2 text-sm text-gray-100 rounded-md absolute left-1/2 
                -translate-x-1/2 translate-y-full opacity-0 mt-1 mx-auto whitespace-nowrap">{{ rse.firstname }}
                {{ rse.lastname }}</span>
            </div>
          </dd>
        </div>
        <Menu as="div" class="relative flex gap-x-2.5 ml-6">
          ...
          <transition enter-active-class="transition ease-out duration-100"
            ...
            <MenuItems
              class="absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md bg-white py-2 divide-y divide-gray-100 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
              <div class="py-1">
                <MenuItem v-slot="{ active }">
                <div v-on:click="changeCondition(project, 'green')"
                  :class="[active ? 'bg-gray-50' : '', 'flex items-center px-4 py-1 text-sm font leading-6 text-gray-900']">
                  <span class="grow">Stable</span>
                  <CheckIcon v-if="project.condition === 'green'" class="h-5 w-5 text-gray-700" />
                </div>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                <div v-on:click="changeCondition(project, 'amber')"
                  :class="[active ? 'bg-gray-50' : '', 'flex items-center px-4 py-1 text-sm leading-6 text-gray-900']">
                  <span class="grow">At Risk</span>
                  <CheckIcon v-if="project.condition === 'amber'" class="h-5 w-5 text-gray-700" />
                </div>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                <div v-on:click="changeCondition(project, 'red')"
                  :class="[active ? 'bg-gray-50' : '', 'flex items-center px-4 py-1 text-sm leading-6 text-gray-900']">
              ...
              <div class="py-1">
                <MenuItem v-slot="{ active }">
                <a target="_blank" :href="project.hubspotLink"
                  :class="[active ? 'bg-gray-50' : '', 'block px-4 py-1 text-sm leading-6 text-gray-900']">Hubspot</a>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                <a target="_blank" :href="project.clockifyLink"
                  :class="[active ? 'bg-gray-50' : '', 'block px-4 py-1 text-sm leading-6 text-gray-900']">Clockify</a>
                </MenuItem>
              </div>
            </MenuItems>
          </transition>
